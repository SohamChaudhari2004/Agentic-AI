...
def main():
    ...
    # Single image URL
                            if "result_url" in result:
                                ...
                                st.image(result["result_url"], caption="Generated Image", use_column_width=True)

                            # Multiple image URLs
                            elif "result_urls" in result:
                                st.session_state.edited_image = result["result_urls"][0]
                                st.success("✨ Image generated successfully!")
                                st.image(result["result_urls"], caption=["Generated Image"] * len(result["result_urls"]), use_column_width=True)
                            ...
                            # Nested results
                                ...
                                for item in result["result"]:
                                    if isinstance(item, dict) and "urls" in item:
                                        ...
                                        st.success("✨ Image generated successfully!")
                                        st.image(item["urls"], caption="Generated Image", use_column_width=True)
                                        break
                                    elif isinstance(item, list) and len(item) > 0:
                                        st.session_state.edited_image = item[0]
                                        st.success("✨ Image generated successfully!")
                                        st.image(item[0], caption="Generated Image", use_column_width=True)
                                        break
